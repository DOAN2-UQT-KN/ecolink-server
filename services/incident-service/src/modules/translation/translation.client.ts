...
export async function translateText(
  ...
  if (!apiKey) {
    console.warn(
      "[incident-service] INTERNAL_AI_API_KEY is not set; skipping translation",
    );
  ...
  try {
    ...
    if (!res.ok) {
      // Read body for diagnostics (FastAPI returns {"detail": "..."} on errors).
      const bodyText = await res.text().catch(() => "");
      console.warn(
        `[incident-service] translateText HTTP ${res.status} from ai-service: ${bodyText.slice(0, 500)}`,
      );
      return { vi: trimmed, en: trimmed };
    }
    ...
    console.warn(
      `[incident-service] translateText network/parse error: ${err instanceof Error ? err.message : String(err)}`,
    );
    return { vi: trimmed, en: trimmed };
  }
}
