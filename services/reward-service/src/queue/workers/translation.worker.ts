...
const TRANSLATION_WORKER_LABEL = "TRANSLATE_TEXT";

/**
 * Translates a batch of `(sourceText, vi/en field)` tuples and writes the
 * results back into the matching reward-service resource row. Runs in the
 * dedicated reward-translation SQS queue so request handlers no longer block
 * on the AI translation service.
 */
export class TranslationWorker extends QueueWorker {
  protected readonly jobType = TRANSLATION_WORKER_LABEL;
  ...
  protected jobTypeMatches(messageJobType: string): boolean {
    return messageJobType === TRANSLATE_TEXT_JOB_TYPE;
  }
  ...
  private async applyUpdate(
    ...
    switch (resourceType) {
      case TranslationResourceType.GIFT:
        await prisma.gift.update({
          where: { id: resourceId },
          data: data as never,
        });
        return;
      case TranslationResourceType.DIFFICULTY:
